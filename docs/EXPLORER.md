# Group Recommendation Explorer — how it's built

The explorer ([`group_rec_explorer.html`](group_rec_explorer.html)) is a single
self-contained page generated by [`scripts/build_explorer.py`](../scripts/build_explorer.py).
...
* A group's **interactions** are *derived, not simulated*: the **consensus items** are
  those rated `>= 4` by `>= 2` of the three members (a deterministic function of the real
  ratings). We keep groups with `>= 3` consensus items; deep models train on them and the
  first consensus item is the marked positive (`✓`).
