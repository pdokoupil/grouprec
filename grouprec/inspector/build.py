...
except ModuleNotFoundError as exc:  # optional [torch] extra
    raise SystemExit(
        "grouprec-build-inspector needs PyTorch. Install it with: pip install 'grouprec[torch]'"
    ) from exc
...
gim.group_scores(members, cands, member_weights=w)</pre>
  ...
  (<code>grouprec.inspector.build</code>, ~400 lines) wraps those calls with: parsing MovieLens
  ...
  <p style="margin-top:14px">Reproduce with <code>grouprec-build-inspector</code>. Full write-up:
